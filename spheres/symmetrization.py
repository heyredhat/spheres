...
def symmetrize(pieces):
    ...
    """
    return sum([qt.tensor(*[pieces[i] for i in perm]) for perm in permutations(range(len(pieces)))]).unit() if type(pieces) == list \
        else sum([pieces.permute(perm) for perm in permutations(range(len(pieces.dims[0])))]).unit()
...
def spin_sym(spin, map=None):
    ...
    ----------
        spin : qt.Qobj
            Pure or mixed spin state.
    
    Returns
    -------
        sym : qt.Qobj
            Pure or mixed state of 2j symmetrized qubits.
    """
    j = (spin.shape[0]-1)/2
    if not map:
        map = spin_sym_map(j)
    return map*spin if spin.type != 'oper' else map*spin*map.dag()
...
def sym_spin(sym, map=None):
    ...
    if not map:
        map = spin_sym_map(j)
    return map.dag()*sym if sym.type != 'oper' else map.dag()*sym*map
